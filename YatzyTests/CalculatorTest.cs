...

{
    public class CalculatorTest
    {
        ...
        [Theory]
        [InlineData(2, 1, 1, 1, 4, 6)]
        [InlineData(8, 1, 4, 1, 4, 4)]
        public void PairsTest(int expected, int a, int b, int c, int d, int e)
        {
            var calc = new ScoreCalculator();
            var dice = SetUpDice(a, b, c, d, e);

            Assert.Equal(expected, calc.Pairs(dice));
        }
